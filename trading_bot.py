...
class PionexTradingBot:
    ...
    def test_connection(self):
        ...
        try:
            ...
            if response.get('result', False):
                ...
                logging.info(f"帳戶餘額: {response.get('data', {}).get('balances', [])}")
                return True
            else:
    ...
    def get_market_data(self, symbol, timeframe='1h', limit=100):
        ...
        try:
            ...
            # 獲取正確的時間間隔格式
            formatted_interval = interval_mapping.get(timeframe)
            if not formatted_interval:
                logging.warning(f"不支援的時間間隔 {timeframe}，使用默認值 1h")
                formatted_interval = '1h'
            ...
            logging.info(f"正在獲取市場數據: {formatted_symbol}, 時間間隔: {formatted_interval}")
            response = self.make_request('GET', '/api/v1/market/klines', params)
    ...
    def calculate_indicators(self, df):
        ...
        df['StochRSI'] = (df['RSI'] - df['RSI'].rolling(window=14).min()) / \
                         (df['RSI'].rolling(window=14).max() - df['RSI'].rolling(window=14).min())
    ...
    def update_investment_amounts(self, total_usdt, btc_percentage=0.6):
        """更新配置文件中的投資金額"""
        self.config['grid_settings']['BTC/USDT']['investment_amount'] = round(total_usdt * btc_percentage, 2)
        self.config['grid_settings']['ETH/USDT']['investment_amount'] = round(total_usdt * (1 - btc_percentage), 2)
        self.save_config()
        ...
        try:
            ...
            for pair in self.trading_pairs:
                ...
                if signals.get('should_trade', False):
                    # 計算倉位大小
                    position_size = self.optimize_position_management(pair, sentiment)
                    
                    # 獲取動態止盈止損點
                    targets = self.risk_manager.calculate_dynamic_targets(pair, sentiment)
    ...
    def execute_long_strategy(self, symbol, grid_levels, investment_amount, signal_strength):
        ...
        try:
            ...
            # 根據信號強度調整網格參數
            grid_count = int(grid_levels['grid_count'] * (1 + signal_strength * 0.1))
            price_range = grid_levels['price_range_percentage'] * (1 + signal_strength * 0.05)
            ...
            for i in range(grid_count):
                ...
                order_amount = self.calculate_position_size(investment_amount, grid_count, signal_strength)
                
                if grid_price < current_price:
                    self.place_buy_order(symbol, order_amount * 1.2, grid_price)
                else:
    ...
    def execute_short_strategy(self, symbol, grid_levels, investment_amount, signal_strength):
        ...
        try:
            ...
            for i in range(grid_count):
                ...
                if grid_price > current_price:
                    self.place_sell_order(symbol, order_amount * 1.2, grid_price)
                else:
    ...
    def calculate_position_size(self, investment_amount, grid_count, signals):
        ...
        # 根據信號強度調整倉位
        signal_strength = 0
        if signals['trend_strong_up']: signal_strength += 0.2
        if signals['trend_strong_down']: signal_strength -= 0.2
        if signals['rsi_oversold'] and signals['kdj_oversold']: signal_strength += 0.15
        if signals['rsi_overbought'] and signals['kdj_overbought']: signal_strength -= 0.15
    ...
    def print_trading_stats(self):
        ...
        win_rate = (self.profit_tracker['win_trades'] / self.profit_tracker['total_trades'] * 100) if self.profit_tracker['total_trades'] > 0 else 0
            ...
            if self.check_add_position_conditions(symbol, current_price):
                ...
                if order['status'] == 'filled':
                    ...
                    position['average_price'] = (
                        position['total_investment'] / position['current_position']
                    )
                    
                    # 記錄加倉操作
                    logging.info(f"加倉成功 - {symbol}: 數量={amount}, 價格={current_price}")
        ...
        try:
            ...
            if self.check_reduce_position_conditions(symbol, current_price):
                ...
                if order['status'] == 'filled':
                    ...
                    position = self.position_manager[symbol]
                    position['current_position'] -= amount
                    position['total_investment'] -= (amount * position['average_price'])
                    ...
                    # 記錄減倉操作
                    logging.info(f"減倉成功 - {symbol}: 數量={amount}, 價格={current_price}")
    ...
    def optimize_position_management(self, symbol, sentiment):
        ...
        try:
            ...
            # 綜合計算最終倉位
            final_position = base_position * sentiment_factor * volatility_factor * trend_factor
    ...
    def optimize_investment_allocation(self, total_investment):
        ...
        try:
            # 獲取前五大交易量幣種
            top_pairs = self.market_analyzer.get_top_volume_pairs(self.exchange)
            
            # 分析每個交易對的歷史表現
            performance_data = {}
            for pair, volume in top_pairs:
                # 獲取歷史數據
                historical_data = self.get_market_data(pair, timeframe='1d', limit=30)
    ...
    def start_market_data_update(self):
        ...
        try:
            def update_market_data():
                try:
                    # 使用正確的交易對格式
                    data = self.get_market_data('BTC_USDT', '1m')  # 使用底線格式
                    if data is not None:
    ...
    def get_account_status(self):
        ...
        try:
            # 只使用餘額接口
            balance_response = self.make_request('GET', '/api/v1/account/balances')
            ...
            # 計算總資產和可用餘額
            total_balance = sum(float(b['free']) + float(b['frozen']) for b in balances if b['coin'] == 'USDT')
            available_balance = sum(float(b['free']) for b in balances if b['coin'] == 'USDT')

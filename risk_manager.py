...
class RiskManager:
    ...
    def check_position_risk(self, symbol, position_size, current_price):
        ...
        initial_price = self.positions.get(
            symbol, {}).get('entry_price', current_price)
...
class EnhancedRiskManager:
    ...
    def calculate_dynamic_stop_loss(self, position, volatility):
        ...
        if position['unrealized_profit'] > 0:
            profit_factor = min(
                position['unrealized_profit'] / position['entry_price'], 0.03)
            adjusted_stop_loss = max(
                adjusted_stop_loss - profit_factor, base_stop_loss * 0.5)
...
class AdvancedRiskManager:
    ...
    def calculate_dynamic_targets(self, symbol, sentiment):
        """計算動態止盈止損點"""
        try:
            # 獲取市場數據，使用預設的時間間隔（1分鐘）
            data = self.get_market_data(symbol, '1')  # 使用 1 分鐘的時間間隔
            if data is None:
                return None
